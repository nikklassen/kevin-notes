...
class ListsSuite extends FunSuite {
  ...
  test("max of identical numbers") {
    assert(max(List(3,3,3,3,3)) === 3)
  }

  test("max of negatives") {
    assert(max(List(-3,-2,-1)) === -1)
  }

  test("max across negatives") {
    assert(max(List(-3,0,3)) === 3)
  }
